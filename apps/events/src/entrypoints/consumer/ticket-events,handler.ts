...
export class TicketEventsHandler {
  ...
  private subscribeToOrderChanges(): void {
    this.rmqService.consume('order.ticket.purchased', this.handleTicketPurchased.bind(this));
    this.rmqService.consume('order.ticket.refunded', this.handleTicketRefunded.bind(this));
  }

  private async handleTicketPurchased(msg: amqp.ConsumeMessage | null): Promise<void> {
    if (!msg) return;
    ...
    try {
      ...
      this.logger.log(`Received order.ticket.purchased for eventId: ${eventId}, quantity: ${quantity}`);
      ...
      this.logger.log(`Updated available tickets for event ${eventId}: ${currentTickets} -> ${newTicketCount}`);
    } catch (error) {
      this.logger.error('Error processing order.ticket.purchased event', error);
    }
  }

  private async handleTicketRefunded(msg: amqp.ConsumeMessage | null): Promise<void> {
    if (!msg) return;
    ...
    try {
      ...
      this.logger.log(`Received order.ticket.refunded for eventId: ${eventId}, quantity: ${quantity}`);
