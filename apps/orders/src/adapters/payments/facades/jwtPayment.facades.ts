...
export class JwtPaymentAdapter implements OnModuleInit {
  ...
  constructor(
    ...
    this.merchantKey = this.configService.getOrThrow<string>(
      'PAYMENT_GATEWAY_MERCHANT_KEY',
    );

    if (!this.apiUrl) {
      this.logger.error(
        'PAYMENT_GATEWAY_URL is not defined in the environment variables',
      );
    }
  }
  ...
  async getJwtToken(
    ...
    try {
      ...
      this.logger.error(
        `JWT token retrieval failed: ${error.message}`,
        error.stack,
      );
      return { status: false };
    }
